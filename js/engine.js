...
KQ.TypingSession=class{
  ...
  backspace(){
    ...
    if(this.states[this.pos]===KQ.WRONG){this.states[this.pos]=KQ.PENDING;this.typedChars[this.pos]='';this.updateWpm();return true}
  ...
  currentErrors(){return !this.done&&this.states[this.pos]===KQ.WRONG?1:0}
  ...
  stats(){const ms=this.elapsedMs(),correct=this.correctCount();return{wpm:this.wpm,accuracy:this.keystrokes?Math.max(0,Math.round((correct/this.keystrokes)*100)):100,mistakes:this.totalErrors,currentErrors:this.currentErrors(),keystrokes:this.keystrokes,typed:this.pos,correct,seconds:Math.round(ms/1000),done:this.done,progress:this.text.length?this.pos/this.text.length:0,expected:this.expected}}
};