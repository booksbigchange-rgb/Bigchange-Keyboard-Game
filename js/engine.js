...
KQ.TypingSession=class{
  ...
  input(ch){
    ...
    const correct=ch===this.text[this.pos];this.keystrokes++;this.typedChars[this.pos]=ch;this.states[this.pos]=correct?KQ.CORRECT:KQ.WRONG;
    if(!correct)this.totalErrors++;this.pos++;this.updateWpm();if(this.pos>=this.text.length){this.finish();return'complete'}return correct?'correct':'wrong';
  }
  backspace(){if(this.done||this.pos<=0)return false;this.pos--;this.states[this.pos]=KQ.PENDING;this.typedChars[this.pos]='';this.updateWpm();return true}
  ...
  stats(){const ms=this.elapsedMs(),correct=this.correctCount();return{wpm:this.wpm,accuracy:this.keystrokes?Math.max(0,Math.round(((this.keystrokes-this.totalErrors)/this.keystrokes)*100)):100,mistakes:this.totalErrors,currentErrors:this.currentErrors(),keystrokes:this.keystrokes,typed:this.pos,correct,seconds:Math.round(ms/1000),done:this.done,progress:this.text.length?this.pos/this.text.length:0,expected:this.expected}}
};